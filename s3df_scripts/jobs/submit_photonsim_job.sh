#!/bin/bash
# Script to submit PhotonSim jobs to S3DF
# Usage: ./submit_photonsim_job.sh -p <particle> -n <nevents> -e <energy> -o <output_dir> [-f <filename>]
...
MACRO_FILE="${FULL_OUTPUT_DIR}/run_${PARTICLE}_${ENERGY_INT}MeV_${NEVENTS}events.mac"
echo "Creating macro file: ${MACRO_FILE}"

cat > "${MACRO_FILE}" << EOF
# PhotonSim macro for ${PARTICLE} at ${ENERGY} MeV
# Generated by submit_photonsim_job.sh
...
JOB_SCRIPT="${FULL_OUTPUT_DIR}/run_photonsim.sh"
...
SLURM_SCRIPT="${FULL_OUTPUT_DIR}/submit_job.sbatch"
echo "Creating SLURM submission script: ${SLURM_SCRIPT}"

# Create job name with particle, energy, and timestamp
JOB_NAME="photonsim_${PARTICLE}_${ENERGY_INT}MeV_$(date +%Y%m%d_%H%M%S)"
...
#SBATCH --output=${FULL_OUTPUT_DIR}/job-%j.out
#SBATCH --error=${FULL_OUTPUT_DIR}/job-%j.err
#
#SBATCH --ntasks=1
