#!/bin/bash
# Script to submit multiple PhotonSim jobs in batch with individual photon storage enabled
# Usage: ./submit_photonsim_batch_individual.sh -c <config_file>
...
while getopts "c:sth" opt; do
    case $opt in
        ...
        h) echo "Usage: $0 -c <config_file> [-s] [-t]"
           ...
           echo "  mu- 100 1050 /path/to/your/output output_job001.root"
           echo "  mu- 100 1050 /path/to/your/output output_job002.root"
           echo ""
           echo "NOTE: This script uses submit_photonsim_job_individual.sh"
           echo "      which enables individual photon/edep storage."
           exit 0;;
...
echo "Using submit_photonsim_job_individual.sh (individual photon storage ENABLED)"
...
while IFS=' ' read -r particle nevents energy output_dir filename || [ -n "$particle" ]; do
    ...
    # Prepare the job using the individual storage script
    CMD="${SCRIPT_DIR}/submit_photonsim_job_individual.sh -p $particle -n $nevents -e $energy -o $output_dir -f $filename"
    echo "Executing: $CMD"
    $CMD
    ...
    if [ $? -eq 0 ]; then
        ...
        SLURM_SCRIPT="${output_dir}/${particle}/${ENERGY_INT}MeV/submit_job_${filename%.root}.sbatch"
