#!/bin/bash
# Script to submit PhotonSim jobs with UNIFORM energy distribution
# Usage: ./submit_photonsim_job_uniform.sh -p <particle> -n <nevents> -m <min_energy> -M <max_energy> -o <output_dir> [-f <filename>]
...
while getopts "p:n:m:M:o:f:h" opt; do
    case $opt in
        p) PARTICLE="$OPTARG";;
        n) NEVENTS="$OPTARG";;
        m) MIN_ENERGY="$OPTARG";;
        ...
        h) echo "Usage: $0 -p <particle> -n <nevents> -m <min_energy> -M <max_energy> -o <output_dir> [-f <filename>]"
...
# Generated by submit_photonsim_job_uniform.sh
...
echo "Starting PhotonSim job (UNIFORM ENERGY, INDIVIDUAL PHOTON STORAGE)"
...
JOB_NAME="photonsim_uniform_${PARTICLE}_${MIN_ENERGY_INT}-${MAX_ENERGY_INT}MeV_$(date +%Y%m%d_%H%M%S)"
