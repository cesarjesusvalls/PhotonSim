...
Uses uproot (pure-Python ROOT reader) — no PyROOT / C++ ROOT install
required, so it runs under any Python 3.x.
"""
...
def analyze_energy_scan(scan_directory):
    ...
    for i, root_file in enumerate(root_files):
        ...
        try:
            ...
            print(f"  Error: Cannot open {root_file}: {e}")
            continue

        # Get the photon time vs distance histogram
        if "PhotonHist_TimeDistance" not in f:
            print(f"  Error: PhotonHist_TimeDistance not found in {root_file}")
            f.close()
            continue
        hist = f["PhotonHist_TimeDistance"]
        ...
        # uproot exposes a TH2 as: .values() (nx, ny float64 array, 0-indexed, no flow),
        # axis(0)/axis(1).centers() (length-nx / length-ny center arrays).
        hist_values = hist.values()           # shape (n_bins_x, n_bins_y)
        x_centers = hist.axis(0).centers()    # length n_bins_x
        y_centers = hist.axis(1).centers()    # length n_bins_y
        n_bins_x = hist_values.shape[0]
        n_bins_y = hist_values.shape[1]

        # PyROOT's hist.GetEntries() returns fEntries (incremented by Fill()); uproot
        # exposes the same field via member("fEntries").
        n_entries = float(hist.member("fEntries"))
        print(f"  Photons: {n_entries:,.0f}")

        # Create profiles: mean time vs distance AND photon count vs distance.
        # Loop transliterated 1:1 from the PyROOT version so summation order
        # matches and outputs are bit-exact.
        distances_time = []
        mean_times = []
        distances_count = []
        photon_counts = []
        ...
        for bin_x in range(1, n_bins_x + 1):
            ...
            for bin_y in range(1, n_bins_y + 1):
                content = hist_values[bin_x - 1, bin_y - 1]
                if content > 0:
                    time = y_centers[bin_y - 1]
